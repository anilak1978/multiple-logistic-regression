...
This is an observational study. I would reprhase the question: "Is there a relationship between beaty and course of evaluation?" 
...
The distribution is left skewed. There are higher evaluation scores than lower. Obviously students are happy with their professors. 
...
```{r}
# variables selected 'rank' and 'age'

library(ggplot2)
ggplot(evals, aes(rank, age))+
  geom_boxplot()


```

Median of teaching rank is around 50. Tenure track median age is much lower around 35 and tenured rank median is around 53. 
...
There are more observations than the points. 
...
Initially, we were looking at many observations under within one dot. Jitter helped us display the observations. 
...
$Score=3.88034+0.06664*btyavg$

For each additional beaty avg increase , score increases additional 0.06664
...
There is no linear relationship. The distribution of the residuals are normal. 
...
The residuals distribution is not normal. It deviates from the line on top end of the line. 
...
The distribution of the residuals are left skewed. 

8.  Is `bty_avg` still a significant predictor of `score`? Has the addition
    of `gender` to the model changed the parameter estimate for `bty_avg`?
    
Adding the gender variable to the model, btw_avg is still a significant factor. The p value is
smaller with the adition of the gender variable.
...
$\widehat{score} = \hat{\beta}_0 + \hat{\beta}_1 \times bty\_avg + \hat{\beta}_2$

For two professorst who received the same beauty rating, male has higher course evaluation score.
...
R splits the rank variable into 3 levels (teaching, tenure track and tenured) and each level has the equation.
...
I think "`cls_did_eval` variable (number of students in class who completed evaluation) wouldnt have any association with the professor score. Hence, would expect the highest p-value in this model. 
...
We didnt include the cls_did_eval variable to our model. Having said that cls_per_eval also does not have the highest p score. cls_profssingle (number of professors teaching sections in course in sample: single) has the highest p score. My assumption was not correct.

13. Interpret the coefficient associated with the ethnicity variable.

All else held constant, the score of the professors that are not minority tends to be 0.1234929 higher.
...
Some of the coefficients for certain variables have changed. This means the cls_prof variable is dependent on other variables.
...
```{r}
model_2 <- lm(score ~ gender + age + cls_perc_eval 
...
score <- function(gender, age, cls_perc_eval, cls_credits, bty_avg, pic_color){
...
bckwrd <- score(1, evals$age, evals$cls_perc_eval, 1, evals$bty_avg, 1)

comparison <- data.frame(evals$score, bckwrd, bckwrd - evals$score)
head(comparison)

```

Variables selected: gender, age, cls_perc_eval, cls_credits, btv_avg, pic_color
...
The residual distribution is still not normal and left skewed.
...
I think it will have an impact on independence between variables condition. We dont know how many courses one professor teaches.
...
Professor would be male with black and white photo and teaches in less credit classes.

19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?

This is an observational study that is applied to one university; so i wouldnt generalize my conclusion to apply
to professors at other universities. 
